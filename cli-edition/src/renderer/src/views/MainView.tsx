...
export function MainView() {
  ...
  // 未認証系の案内表示中に「最終更新: HH:MM:SS」が残っていると、認証済みなのか
  // どうかユーザーが混乱するため、案内表示中はフッターの時刻も隠す（Web 版と同仕様）。
  const showLoginPrompt = state.error === 'unauthenticated' || state.error === 'session_expired'

  const usageContent = (
    <>
      {state.error === 'unauthenticated' ? (
        <CliSetupGuide t={t} onRetry={handleRefresh} retrying={refreshing} />
      ) : state.error === 'session_expired' ? (
        <SessionExpiredView t={t} onRetry={handleRefresh} retrying={refreshing} />
      ) : state.error ? (
      ...
      <div className="px-4 py-3">
        ...
        <div className="mb-2">
          <div className="inline-flex items-center gap-2 rounded-full bg-gray-100 dark:bg-[#23232a] px-3 py-1.5 text-xs text-gray-600 dark:text-gray-300">
            ...
            {state.accountEmail && (
              <span className="truncate max-w-[280px]">{state.accountEmail}</span>
            )}
